...
public:
    ...
    bool init() {
        ...
        participantQos.name("MotorQueryResponse");
        ...
        queryTopic = participant->create_topic("QueryTopic", "Query_msg", TOPIC_QOS_DEFAULT);

        responseTopic = participant->create_topic("ResponseTopic", "Response_msg", TOPIC_QOS_DEFAULT);
    ...
    // Define the checkForNewQuery function
    bool checkForNewQuery(Query_msg& queryMsg) {
        // Implement the logic to check and retrieve new Query messages
        // Return true if a new message is available, false otherwise
        // Update queryMsg with the new message details
